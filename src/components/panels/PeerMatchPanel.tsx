...
// const FILIERES = [
//   { id: 'IISI',   nom: "Ingénierie Systèmes Informatiques", icon: '💻', color: 'from-blue-500 to-indigo-600'   },
//   { id: 'MGE',    nom: "Management des Entreprises",         icon: '📊', color: 'from-emerald-500 to-teal-600' },
...
// const FILIERES = [
//   { id: 'IISI',   nom: "Ingénierie Systèmes Informatiques", icon: '💻' },
//   { id: 'MGE',    nom: "Management des Entreprises",         icon: '📊' },
...
//   { id: 'MDI',   nom: "Management et Développement International", icon: '🌍' },
//   { id: 'MRI',   nom: "Management et Relations Internationales",   icon: '🤝' },
// ];
...
// // Métadonnées visuelles par filière
// const FILIERE_META: Record<string, { icon: string; nom: string }> = {
//   IISI:  { icon: '💻', nom: "Ingénierie Systèmes Informatiques"       },
//   MGE:   { icon: '📊', nom: "Management des Entreprises"               },
//   MDI:   { icon: '🌍', nom: "Management et Développement International"},
//   IISIC: { icon: '🤖', nom: "IA & Systèmes d'Information"              },
//   IISRT: { icon: '📡', nom: "Réseaux & Télécommunications"             },
//   FACG:  { icon: '💰', nom: "Finance, Audit & Contrôle"                },
//   MRI:   { icon: '🤝', nom: "Management et Relations Internationales"  },
// };
...
//   // Charger les filières accessibles + filière recommandée depuis l'API
//   useEffect(() => {
//     const uid = getUid();
//     fetch(`${API}/api/peermatch/filieres`, {
//       credentials: 'include',
//       headers: uid ? { 'X-User-Id': uid } : {},
//     })
//       .then(r => r.json())
...
//         setFilieres(list);
//         // Filière recommandée : d'abord depuis l'API, sinon depuis le FitScore
//         const rec = data.filiere_recommandee
//           || fitscore?.classement?.[0]?.filiere_id
//           || fitscore?.meilleure_filiere
//           || (list[0]?.id ?? null);
//         setSuggestedId(rec);
//         setSelected(rec);
//       })
//       .catch(() => {
//         // Fallback sur FitScore local
//         const rec = fitscore?.classement?.[0]?.filiere_id || fitscore?.meilleure_filiere || null;
//         setSuggestedId(rec);
//         setSelected(rec);
//       })
...
//   // Vérifier si une demande existe déjà en localStorage
//   useEffect(() => {
//     const savedId = localStorage.getItem('peermatch_demande_id');
//     if (savedId) {
//       setDemandeId(savedId);
...
//           setDemandeId(data.demande_id);
//         }
//         // Forcer statut en_attente — coordonnées visibles SEULEMENT après traitement admin
//         setDemande({ statut: 'en_attente', filiere: selected });
//         setSent(true);
//       }
//     } catch {
...
//   // Coordonnées UNIQUEMENT si statut = traite
//   const ambassadeurAssigne = demande?.statut === 'traite' ? demande?.ambassadeur_nom   : null;
//   const emailContact       = demande?.statut === 'traite' ? demande?.ambassadeur_email : null;
//   const waContact          = demande?.statut === 'traite' ? demande?.ambassadeur_wa    : null;
...
//           <p className="text-[11px] font-medium text-slate-600 dark:text-slate-300">
//             Recommandation SAMI :{' '}
//             <span className="text-orange-600 dark:text-orange-400 font-black">filière {suggestedId}</span>
//             {' '}— un ambassadeur peut te répondre.
//           </p>
//         </div>
//       )}

//       {/* Grille filières dynamiques */}
//       {!sent && (
//         <>
//           <div>
//             <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">
...
//                 Complète ton profil pour voir les filières disponibles.
//               </div>
//             ) : (
//               <div className="grid grid-cols-3 gap-2">
//                 {filieres.map(f => {
//                   const meta = FILIERE_META[f.id] || { icon: '🎓', nom: f.nom };
//                   return (
//                     <button
//                       key={f.id}
...
//                         selected === f.id ? 'text-orange-500' : 'text-slate-500'
//                       )}>
//                         {f.id}
//                       </div>
//                       <div className={cn(
...
//           {error && (
//             <div className="p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl text-xs text-rose-600 text-center">
//               {error}
//             </div>
//           )}
...
//               <ActionBtn
//                 onClick={contact}
//                 disabled={!selected || loadingFilieres}
//                 className="w-full h-14 rounded-2xl bg-[#006666] text-white font-bold text-sm shadow-2xl transition-all active:scale-95 group"
//               >
//                 <Send size={18} className="mr-2 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
//                 Envoyer la demande
...
//                     Ambassadeur SUPMTI · {demande?.filiere || selected}
//                   </p>
//                 </div>
//               </div>
//               <div className="flex flex-col gap-2">
//                 {waContact && (
//                   <a
//                     href={`https://wa.me/${waContact.replace(/\D/g, '')}`}
//                     target="_blank" rel="noopener noreferrer"
//                     className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] text-white rounded-xl text-xs font-bold hover:opacity-90 transition-all"
//                   >
//                     <Phone size={14} /> Contacter via WhatsApp <ExternalLink size={12} />
...
//                 {emailContact && (
//                   <a
//                     href={`mailto:${emailContact}?subject=Peer Match ${demande?.filiere || selected}`}
//                     className="flex items-center justify-center gap-2 w-full py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 hover:opacity-90 transition-all"
//                   >
//                     <Mail size={14} /> Envoyer un Email
//                   </a>
...
//                 </a>
//                 <a
//                   href={`mailto:contact@supmtimeknes.ac.ma?subject=Peer Match ${selected}`}
//                   className="flex items-center justify-center gap-2 w-full py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300"
//                 >
//                   <Mail size={14} /> Email SUPMTI
//                 </a>
...
interface FiliereInfo { id: string; nom: string; niveau: string; }
interface DemandeStatus { statut: string; filiere: string; ambassadeur_nom?: string; ambassadeur_email?: string; ambassadeur_wa?: string; }
...
export const PeerMatchPanel = () => {
  ...
  useEffect(() => {
    const uid = getUid();
    fetch(`${API}/api/peermatch/filieres`, { credentials: 'include', headers: uid ? { 'X-User-Id': uid } : {} })
      .then(r => r.json())
      .then(data => {
        const list: FiliereInfo[] = data.filieres || [];
        setFilieres(list);
        const rec = data.filiere_recommandee || fitscore?.classement?.[0]?.filiere_id || fitscore?.meilleure_filiere || (list[0]?.id ?? null);
        setSuggestedId(rec); setSelected(rec);
      })
      .catch(() => {
        const rec = fitscore?.classement?.[0]?.filiere_id || fitscore?.meilleure_filiere || null;
        setSuggestedId(rec); setSelected(rec);
  ...
  useEffect(() => {
    const savedId = localStorage.getItem('peermatch_demande_id');
    if (savedId) { setDemandeId(savedId); setSent(true); checkStatut(savedId); }
  }, []);
  ...
  const checkStatut = async (id?: string) => {
    ...
    try {
      const uid = getUid();
      const res = await fetch(`${API}/api/peermatch/statut/${dId}`, { credentials: 'include', headers: uid ? { 'X-User-Id': uid } : {} });
      if (res.ok) { const data = await res.json(); setDemande(data); }
    } catch { /* silencieux */ }
    finally { setChecking(false); }
  };
  ...
  const contact = async () => {
    ...
    } catch { setError(t('peermatch','error_send')); }
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-6">
      ...
      <div className="text-center py-6 bg-gradient-to-b from-slate-50 to-transparent dark:from-white/[0.03] rounded-t-3xl border-x border-t border-slate-100 dark:border-white/[0.05]">
        ...
        <h3 className="font-black text-lg text-slate-800 dark:text-white px-6">{t('peermatch','header_title')}</h3>
        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 px-8 leading-relaxed italic">{t('peermatch','header_quote')}</p>
      </div>
      ...
      {suggestedId && !sent && (
        <div className="mx-1 p-3.5 bg-orange-500/5 border border-orange-500/20 rounded-2xl flex items-center gap-3 animate-in slide-in-from-left duration-700">
          ...
          <p className="text-[11px] font-medium text-slate-600 dark:text-slate-300">
            {t('peermatch','sami_rec')} :{' '}
            <span className="text-orange-600 dark:text-orange-400 font-black">{t('panels','fitscore_title')} {suggestedId}</span>
            {' '}{t('peermatch','sami_rec_suffix')}
          </p>
        </div>
      )}
      ...
      {!sent && (
        <>
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">{t('peermatch','choose_program')}</p>

            {loadingFilieres ? (
              <div className="flex items-center justify-center py-8 gap-3 text-slate-400">
                <Loader2 size={18} className="animate-spin" />
                <span className="text-xs">{t('peermatch','loading_filieres')}</span>
              </div>
            ) : filieres.length === 0 ? (
              <div className="text-center py-6 text-slate-400 text-xs bg-slate-50 dark:bg-white/[0.02] rounded-2xl border border-dashed border-slate-200 dark:border-white/[0.05]">
                {t('peermatch','no_filieres')}
              </div>
            ) : (
              <div className="grid grid-cols-3 gap-2">
                {filieres.map(f => {
                  ...
                  return (
                    <button key={f.id} onClick={() => setSelected(f.id)}
                      ...
                      <div className="text-xl mb-1 group-hover:scale-125 transition-transform duration-300">{meta.icon}</div>
                      <div className={cn('text-[10px] font-black tracking-tighter', selected === f.id ? 'text-orange-500' : 'text-slate-500')}>{f.id}</div>
                      <div className={cn("text-[8px] font-bold mt-0.5", f.niveau === 'BAC+5' ? 'text-purple-400' : 'text-emerald-400')}>{f.niveau}</div>
                    </button>
                  );
                })}
              </div>
          ...
          {error && <div className="p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl text-xs text-rose-600 text-center">{error}</div>}

          <div className="pt-2">
            {loading ? (
              <div className="flex flex-col items-center gap-3 py-4">
                <div className="w-8 h-8 border-2 border-slate-200 dark:border-white/10 border-t-orange-500 rounded-full animate-spin" />
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse">{t('peermatch','btn_sending')}</p>
              </div>
            ) : (
              <ActionBtn onClick={contact} disabled={!selected || loadingFilieres}
                className="w-full h-14 rounded-2xl bg-[#006666] text-white font-bold text-sm shadow-2xl transition-all active:scale-95 group">
                <Send size={18} className="mr-2 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                {t('peermatch','btn_send')}
              </ActionBtn>
            )}
          </div>
        </>
      ...
      {sent && (
        <div className="animate-in zoom-in-95 duration-500 space-y-4">
          <div className={cn("rounded-2xl p-4 border-2 flex items-center gap-3",
            demande?.statut === 'traite'  ? "bg-emerald-500/5 border-emerald-500/20"
            : demande?.statut === 'annule' ? "bg-red-500/5 border-red-500/20"
            :                               "bg-yellow-500/5 border-yellow-500/20"
          )}>
            ...
            <div className="flex-1">
              <p className="font-black text-sm text-slate-800 dark:text-white">
                {demande?.statut === 'traite'  ? t('peermatch','status_treated')
                : demande?.statut === 'annule' ? t('peermatch','status_cancelled')
                :                               t('peermatch','status_pending')}
              </p>
              <p className="text-[10px] text-slate-500 mt-0.5">
                {demande?.statut === 'traite'  ? t('peermatch','desc_treated')
                : demande?.statut === 'annule' ? t('peermatch','desc_cancelled')
                :                               t('peermatch','desc_pending')}
              </p>
            </div>
            <button onClick={() => checkStatut()} disabled={checking}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all shrink-0 disabled:opacity-50">
          ...
          {demande?.statut === 'traite' && ambassadeurAssigne && (
            <div className="bg-emerald-500/5 border-2 border-emerald-500/20 rounded-2xl p-5 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col gap-2">
                {waContact && (
                  <a href={`https://wa.me/${waContact.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] text-white rounded-xl text-xs font-bold hover:opacity-90 transition-all">
                    <Phone size={14} /> {t('peermatch','wa_contact')} <ExternalLink size={12} />
                  </a>
                )}
                {emailContact && (
                  <a href={`mailto:${emailContact}?subject=Peer Match ${demande?.filiere || selected}`}
                    className="flex items-center justify-center gap-2 w-full py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 hover:opacity-90 transition-all">
                    <Mail size={14} /> {t('peermatch','email_contact')}
                  </a>
                )}
              </div>
            </div>
          ...
          {demande?.statut !== 'traite' && demande?.statut !== 'annule' && (
            <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200/50 dark:border-white/[0.05] rounded-2xl p-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('peermatch','waiting_title')}</p>
              <div className="flex flex-col gap-2">
                <a href="https://wa.me/212627802602" target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] text-white rounded-xl text-xs font-bold">
                  WhatsApp SUPMTI <ExternalLink size={12} />
          ...
          <button onClick={() => { setSent(false); setDemande(null); setDemandeId(null); setSelected(suggestedId); localStorage.removeItem('peermatch_demande_id'); }}
            className="w-full py-2 text-[11px] text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
            {t('peermatch','new_request')}
          </button>
        </div>
      )}
      ...
      <div className="bg-slate-50 dark:bg-black/20 border border-slate-200/50 dark:border-white/5 rounded-2xl p-4">
        <p className="text-[10px] font-black text-slate-400 mb-3 uppercase tracking-widest flex items-center gap-2">
          <Phone size={10} /> {t('peermatch','support_title')}
        </p>
        <div className="space-y-2 text-[11px] font-medium text-slate-600 dark:text-slate-400">
          <div className="flex justify-between items-center pb-2 border-b border-slate-200/50 dark:border-white/5">
            <span>{t('peermatch','standard')}</span>
            <span className="text-slate-900 dark:text-white">+212 6 27 80 26 02</span>
          </div>
          <div className="flex justify-between items-center pb-2 border-b border-slate-200/50 dark:border-white/5">
            <span>{t('peermatch','email_direct')}</span>
            <span className="text-slate-900 dark:text-white">contact@supmtimeknes.ac.ma</span>
          </div>
          <div className="flex items-center gap-2 pt-1 opacity-60">
            <div className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            <span className="text-[10px]">{t('peermatch','schedule')}</span>
          </div>
        </div>
      </div>
    </div>
