...
interface BourseEstimee {
  eligible: boolean; pourcentage?: number; label?: string; a_payer?: number; message?: string;
}
interface AdmissionItem {
  filiere_id: string; filiere_nom: string; filiere_niveau?: string; probabilite: number;
  categorie: string; eligible: boolean; raison_ineligibilite?: string;
  bourse_estimee?: BourseEstimee | null; points_forts?: string[]; points_faibles?: string[]; conseils?: string;
}
interface AdmissionRapport {
  classement?: AdmissionItem[]; message_niveau?: string; annee_entree?: string; conseil?: string;
}
  ...
  return (
    <div className="p-4 rounded-2xl border border-slate-100 dark:border-white/[0.06] bg-white dark:bg-slate-900/50 space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            <span className="font-black text-[13px] text-slate-800 dark:text-slate-100">{item.filiere_nom}</span>
            {item.filiere_niveau && (
        ...
        {item.eligible ? (
          <span className={cn("text-[11px] font-black px-2 py-1 rounded-lg border shrink-0", badge)}>{pct}%</span>
        ) : (
          <span className="flex items-center gap-1 text-[10px] font-bold text-red-500 bg-red-500/5 border border-red-500/20 px-2 py-1 rounded-lg shrink-0">
            <XCircle size={10} /> {t('admission','not_eligible')}
          </span>
        )}
      </div>
      {item.eligible && (
      ...
      {item.eligible && item.bourse_estimee?.eligible && item.bourse_estimee.pourcentage && (
        <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">
          <Sparkles size={11} />
          {t('admission','scholarship')} {item.bourse_estimee.pourcentage}% → {item.bourse_estimee.a_payer?.toLocaleString()} DH/an
        </div>
      )}
    </div>
  );
...
export const AdmissionPanel = () => {
  ...
  const calc = async () => {
    ...
    try {
      const data = await getAdmission();
      if (data.error) { setError(data.message || t('panels','complete_profile')); setLoading(false); return; }
      const rapport = data.rapport;
      if (typeof rapport === 'string') setResult({ conseil: rapport });
      else if (rapport && typeof rapport === 'object') setResult(rapport as AdmissionRapport);
      else setError('Résultat inattendu du serveur.');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950/30 p-5 border border-blue-100/50 dark:border-blue-900/20">
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-bold text-slate-800 dark:text-slate-100 tracking-tight">{t('admission','header_title')}</h3>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed max-w-[240px]">
            {t('admission','header_desc')}
          </p>
        </div>
        <GraduationCap size={80} className="absolute -right-4 -bottom-4 text-blue-500/5 dark:text-blue-400/5 -rotate-12" />
      </div>
      ...
      <div className="relative group">
        ...
        <ActionBtn onClick={calc} disabled={loading}
          className={cn("relative w-full py-4 font-bold tracking-tight shadow-xl transition-all active:scale-95",
            result ? "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 shadow-none border-transparent" : "bg-[#006666] text-white"
          )}>
          {loading ? (
            <span className="flex items-center gap-2">{t('admission','btn_loading')}</span>
          ) : (
            <span className="flex items-center justify-center gap-2">
              <DoorOpen size={18} className={cn(result ? "text-slate-400" : "animate-pulse")} />
              {result ? t('admission','btn_rerun') : t('admission','btn_simulate')}
            </span>
          )}
        </ActionBtn>
      </div>
      ...
      {/* Résultats */}
      <div className="min-h-[100px]">
        {loading && <div className="flex flex-col items-center gap-4 py-10"><Spinner label={t('admission','spinner_label')} /></div>}

        {error && (
          <div className="w-full animate-in zoom-in-95 duration-300">
            <ErrorBox message={error} />
            <p className="text-[10px] text-center text-slate-400 mt-4 px-6 italic">{t('admission','error_incomplete')}</p>
          </div>
        )}
        ...
        {!loading && result && (
          <div className="w-full animate-in fade-in slide-in-from-top-4 duration-700 space-y-4">
            <div className="flex items-center gap-2 px-1">
              <Sparkles size={14} className="text-orange-500" />
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('admission','algo_label')}</span>
            </div>
              ...
              <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
                {result.annee_entree && (
                  <div className="flex items-center gap-2 mb-1">
                    <TrendingUp size={12} className="text-blue-500" />
                    <span className="text-[11px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-wider">
                      {t('admission','entrance_label')} {result.annee_entree}
                    </span>
                  </div>
                )}
                {result.message_niveau && <p className="text-[11px] text-slate-500 dark:text-slate-400">{result.message_niveau}</p>}
              </div>
            )}
            ...
            {eligibles.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                  <CheckCircle2 size={12} className="text-emerald-500" />
                  <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">
                    {t('admission','eligible_label')} ({eligibles.length})
                  </span>
                </div>
                {eligibles.map(item => <ProbBar key={item.filiere_id} item={item} />)}
              </div>
            )}
            ...
            {nonEligibles.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                  <XCircle size={12} className="text-red-400" />
                  <span className="text-[10px] font-black text-red-400 uppercase tracking-widest">
                    {t('admission','not_eligible_label')} ({nonEligibles.length})
                  </span>
                </div>
                {nonEligibles.map(item => <ProbBar key={item.filiere_id} item={item} />)}
              </div>
            )}
            ...
            {result.conseil && (
              <ResultCard accent className="border-t-4 border-t-[#006666] shadow-2xl shadow-blue-500/5">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles size={13} className="text-orange-500" />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('admission','council_label')}</span>
                </div>
                <Prose content={result.conseil} />
              </ResultCard>
            )}
            ...
            <div className="mt-4 p-4 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
              <p className="text-[10px] text-slate-400 text-center leading-tight">{t('admission','footer_note')}</p>
            </div>
          </div>
        )}
      </div>
