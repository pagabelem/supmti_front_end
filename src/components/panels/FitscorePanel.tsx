...
export const FitscorePanel = () => {
  ...
  const calc = async () => {
    ...
    try {
      ...
      if (data.error) {
        setError(data.message || t('fitscore', 'insufficient_data'));
        setLoading(false);
        return;
      }
      ...
      setError(t('fitscore', 'engine_error'));
    }
  ...
  const eligibles =
    fitscore?.classement?.filter((f: any) => f.eligible !== false) ?? [];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-6">
      ...
      <div className="p-4 rounded-2xl bg-gradient-to-br from-[#006666]/5 to-blue-500/5 border border-[#006666]/10">
        <div className="flex items-center gap-2 mb-2">
          <BrainCircuit size={18} className="text-[#006666]" />
          <span className="text-[10px] font-black text-[#006666] uppercase tracking-[0.2em]">
            {t('fitscore', 'matching_algorithm')}
          </span>
        </div>

        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
          {t('fitscore', 'intro_before')}{' '}
          <span className="text-slate-700 dark:text-slate-200 font-bold">
            {t('fitscore', 'current_curriculum')}
          </span>
          , {t('fitscore', 'intro_middle')}{' '}
          <span className="text-slate-700 dark:text-slate-200 font-bold">
            {t('fitscore', 'bac_performance')}
          </span>{' '}
          {t('fitscore', 'intro_and')}{' '}
          <span className="text-slate-700 dark:text-slate-200 font-bold">
            {t('fitscore', 'psycho_profile')}
          </span>{' '}
          {t('fitscore', 'intro_after')}
        </p>
      </div>
        ...
        >
          {loading ? (
            <span className="flex items-center gap-2 italic text-sm">
              {t('fitscore', 'syncing_data')}
            </span>
          ) : (
            <span className="flex items-center justify-center gap-2">
              <BarChart3 size={18} />
              {fitscore?.classement
                ? t('fitscore', 'recalculate')
                : t('fitscore', 'calculate')}
            </span>
          )}
        </ActionBtn>
      </div>
      ...
      {loading && (
        <div className="flex flex-col items-center py-12 gap-4 animate-pulse">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            {t('fitscore', 'calculating_probabilities')}
          </p>
        </div>
      )}
          ...
          {eligibles.length > 0 && (
            <div className="relative px-1">
              <div className="flex items-center gap-2 mb-3">
                <Target size={14} className="text-[#006666]" />
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                  {t('fitscore', 'eligible_title')} ({eligibles.length})
                </h4>
              </div>
          ...
          {nonEligibles.length > 0 && (
            <details className="group">
              <summary className="flex items-center gap-2 px-1 cursor-pointer list-none">
                <XCircle size={12} className="text-red-400" />
                <span className="text-[10px] font-black text-red-400 uppercase tracking-widest">
                  {t('fitscore', 'non_eligible_title')} ({nonEligibles.length})
                </span>
                <ChevronRight
                  size={12}
                  className="text-slate-400 ml-auto group-open:rotate-90 transition-transform"
                />
              </summary>
              ...
              <div className="mt-3 space-y-2 px-1">
                {nonEligibles.map((f: any, idx: number) => {
                  ...
                  return (
                    <div
                      key={f.filiere_id || idx}
                      className="p-3 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/[0.04]"
                    >
                      <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-3 text-left">
              ...
              <div>
                <p className="text-[11px] font-black uppercase tracking-widest text-emerald-400">
                  {t('fitscore', 'precision_plus')}
                </p>
                <p className="text-xs text-slate-400">
                  {t('fitscore', 'take_psycho_test')}
                </p>
              </div>
            </div>
