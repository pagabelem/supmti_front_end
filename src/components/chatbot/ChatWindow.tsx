...
export const ChatWindow = () => {
  ...
  const SUGGESTIONS = [
    { icon: '📚', title: t('chat', 'suggestions.filieres'),  desc: t('panels', 'carriere_title'),   msg: t('chat', 'empty_subtitle').split('.')[0] + ' filières' },
    { icon: '💰', title: t('chat', 'suggestions.frais'),     desc: t('panels', 'fitscore_title'),   msg: 'Quels sont les frais de scolarité et les bourses ?' },
    { icon: '🎯', title: t('chat', 'suggestions.admission'), desc: t('panels', 'admission_title'),  msg: t('panels', 'admission_btn') },
    { icon: '📊', title: t('chat', 'suggestions.fitscore'),  desc: t('panels', 'fitscore_btn'),     msg: t('panels', 'fitscore_btn') },
    { icon: '🚀', title: t('chat', 'suggestions.carriere'),  desc: t('panels', 'carriere_btn'),     msg: t('panels', 'carriere_btn') },
    { icon: '🧠', title: t('chat', 'suggestions.psycho'),    desc: t('panels', 'psycho_title'),     msg: t('panels', 'psycho_btn') },
  ];

  return (
    <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-slate-900 pb-36">
      {messages.length === 0 && (
        <div className="flex flex-col items-center text-center px-4 pt-10 pb-6 animate-in fade-in duration-500">
          <div className="w-20 h-20 bg-gradient-to-br from-orange-500 to-orange-600 rounded-[24px] flex items-center justify-center text-4xl mb-5 shadow-2xl shadow-orange-500/30">
            🎓
      ...
      {isTyping && (
        <div className="flex gap-3 items-center px-1">
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm">
            <div className="flex gap-1 items-center">
              {[0, 1, 2].map((i) => (
                <span key={i} className="w-1.5 h-1.5 rounded-full bg-gray-400 dark:bg-slate-500 animate-bounce" style={{ animationDelay: `${i * 0.18}s` }} />
              ))}
            </div>
          </div>
        </div>
