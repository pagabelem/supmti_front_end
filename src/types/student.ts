...
export interface SessionData {
  ...
  chat_actuel_titre:    string;
  nb_messages:          number;
}

// ── Réponses backend ──────────────────────────────────────────

export interface FiliereScore {
  filiere_id:  string;
  filiere_nom: string;
  score_total: number;
}
...
export interface CarriereResult {
  scenario:              string;
  filiere_nom:           string;
  donnees_cles:          { salaire_depart: string; salaire_3ans: string; salaire_7ans: string; taux_insertion: string; };
...
export interface PsychoStartResult {
  message:            string;
  question_actuelle:  number;
  total_questions:    number;
}

export interface PsychoAnswerResult {
  complete?:          boolean;
  message?:           string;
  question_actuelle?: number;
