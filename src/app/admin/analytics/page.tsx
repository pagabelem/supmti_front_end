...
//   IISI:'#006666', MGE:'#CC0000', IISIC:'#7c3aed', IISRT:'#0284c7', FACG:'#d97706', MSTIC:'#059669'
// };
...
//       .catch(()=>setData({
//         fitscore_par_filiere:  {IISI:88.2,MGE:74.5,IISIC:91.3,IISRT:82.7,FACG:69.1,MSTIC:78.4},
...
//         filiere_taux_admission:{IISI:78,MGE:85,IISIC:72,IISRT:80,FACG:88,MSTIC:82},
//       }))
...
//             {Object.entries(data?.filiere_taux_admission||{IISI:78,MGE:85,IISIC:72,IISRT:80,FACG:88,MSTIC:82})
...
// ── Couleurs officielles 7 filières ──────────────────────────
const COLORS_FILIERES: Record<string, string> = {
  IISI:  '#006666',
  IISIC: '#7c3aed',
  IISRT: '#0284c7',
...
const BAC_COLORS = ['#006666','#CC0000','#7c3aed','#0284c7','#d97706','#059669','#f59e0b'];

// ── Données fallback correctes (7 filières, sans MSTIC) ──────
const FALLBACK_DATA = {
  fitscore_par_filiere: {
    IISI: 88.2, IISIC: 91.3, IISRT: 82.7,
    MGE:  74.5, MDI:   71.0, FACG:  69.1, MRI: 68.5,
  },
  langues_utilisees: { fr: 72, ar: 18, en: 10 },
  conversations_par_jour: [
    { date: '10/03', count: 8  },
  ...
  taux_profil_complet:      45,
  moyenne_fitscore_global:  81.2,
  test_psycho_completes:    14,
  bac_distribution:         { SM: 4, PC: 3, SVT: 2, Eco: 1 },
  filiere_taux_admission: {
    IISI: 78, IISIC: 72, IISRT: 80,
    MGE:  85, MDI:   80, FACG:  88, MRI: 75,
  },
};
...
function PieChartSVG({ data }: { data: Record<string, number> }) {
  ...
  const total   = entries.reduce((s, [, v]) => s + v, 0);
  if (!total) return (
    <p className="text-gray-400 dark:text-slate-500 text-sm text-center py-8">Aucune donnée</p>
  );
...
export default function AdminAnalytics() {
  ...
  const maxConv = Math.max(...(data?.conversations_par_jour || []).map((d: any) => d.count), 1);
  const card    = "p-6 rounded-2xl bg-white dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700 shadow-sm";
  const titleCls = "text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest";

  // Nettoyer les données API si elles contiennent encore MSTIC
  const fitscoreData: Record<string, number> = {};
  Object.entries(data?.fitscore_par_filiere || {}).forEach(([k, v]) => {
    if (k !== 'MSTIC' && k !== 'ISI' && k !== 'ME') fitscoreData[k] = v as number;
  });

  const admissionData: Record<string, number> = {};
  Object.entries(data?.filiere_taux_admission || FALLBACK_DATA.filiere_taux_admission).forEach(([k, v]) => {
    if (k !== 'MSTIC' && k !== 'ISI' && k !== 'ME') admissionData[k] = v as number;
  });
    ...
    <div className="p-8 space-y-8 bg-gray-50 dark:bg-slate-900 min-h-full">
        ...
        {[
          {
            ...
            value: `${data?.moyenne_fitscore_global?.toFixed(1) || 0}%`,
            icon: BarChart3, color: 'text-orange-500',
            bg: 'bg-orange-50 dark:bg-orange-500/10',
            border: 'border-orange-100 dark:border-orange-500/20',
          },
          {
            label: 'Tests psycho',
            value: data?.test_psycho_completes || 0,
          ...
          },
          {
            ...
            value: `${data?.taux_profil_complet || 0}%`,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={card}>
          <div className="flex items-center gap-2 mb-6">
            <BarChart3 size={16} className="text-orange-500" />
            <h3 className={titleCls}>FitScore par filière</h3>
          </div>
          <div className="space-y-4">
            {Object.entries(fitscoreData)
              .sort(([, a], [, b]) => (b as number) - (a as number))
              .map(([f, s]: any) => (
                <div key={f} className="flex items-center gap-3">
                  <span className="text-xs font-black text-gray-700 dark:text-slate-300 w-14">{f}</span>
                  <div className="flex-1 h-3 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div
                  ...
                  <span className="text-xs font-bold text-gray-500 dark:text-slate-400 w-12 text-right">
        ...
        <div className={card}>
          <div className="flex items-center gap-2 mb-6">
            <PieChart size={16} className="text-blue-500" />
            <h3 className={titleCls}>Répartition BAC</h3>
          </div>
          <PieChartSVG data={data?.bac_distribution || {}} />
        </div>
        ...
        <div className={card}>
          <div className="flex items-center gap-2 mb-6">
            <TrendingUp size={16} className="text-emerald-500" />
            <h3 className={titleCls}>Taux d'admission estimé</h3>
          </div>
          <div className="space-y-3">
              ...
              .map(([f, t]: any) => (
                <div key={f} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-700"
                      style={{ width: `${t}%`, backgroundColor: COLORS_FILIERES[f] || '#10b981' }}
                    />
                  </div>
                  <span className="text-xs text-emerald-600 dark:text-emerald-400 font-bold w-10 text-right">
                    {t}%
                  </span>
                </div>
              ))}
        ...
        <div className={card}>
          <div className="flex items-center gap-2 mb-6">
            <Globe size={16} className="text-cyan-500" />
            <h3 className={titleCls}>Langues utilisées</h3>
          </div>
          <div className="space-y-4">
            {Object.entries(data?.langues_utilisees || {}).map(([lang, pct]: any) => (
              <div key={lang} className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-gray-500 dark:text-slate-400 w-8 text-right">
                  {pct}%
                </span>
              </div>
            ))}
      ...
      <div className={card}>
        <div className="flex items-center gap-2 mb-6">
          <TrendingUp size={16} className="text-emerald-500" />
          <h3 className={titleCls}>Activité journalière (conversations)</h3>
        </div>
        <div className="flex items-end gap-2 h-36">
          {(data?.conversations_par_jour || []).map((d: any) => (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-1.5 group">
              <span className="text-[10px] text-gray-500 dark:text-slate-400 font-bold">{d.count}</span>
              <div
                className="w-full bg-[#006666] group-hover:bg-emerald-500 rounded-t-lg transition-all cursor-pointer"
                style={{ height: `${Math.max(4, Math.round((d.count / maxConv) * 100))}%` }}
              />
              <span className="text-[9px] text-gray-400 dark:text-slate-500">{d.date}</span>
            </div>
